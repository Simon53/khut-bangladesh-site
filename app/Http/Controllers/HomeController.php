...
public function patchworkProducts()
{
    ...
    // Load banner (ignore menu id)
    $banner = \App\Models\CategoryBanner::first();

    if ($banner) {
        // যদি DB তে full URL থাকে
        if (str_starts_with($banner->banner_image, 'http')) {
            $category->banner = $banner->banner_image;

        // যদি DB তে storage path থাকে
        } else {
            $category->banner = asset('storage/' . $banner->banner_image);
        }

    } else {
        // Default fallback
        $category->banner = asset('assets/images/product_banner.jpg');
    }
...
public function newArrivalsProducts()
{
    ...
    if ($banner) {
        // DB তে যদি full URL থাকে
        if (str_starts_with($banner->banner_image, 'http')) {
            $category->banner = $banner->banner_image;

        // DB তে যদি শুধু path থাকে
        } else {
            $category->banner = asset('storage/' . $banner->banner_image);
        }
